...
def describe_klass(obj):
    ...
    if count==0:
        for name in inspect.classify_class_attrs(obj):
...
def describe_other(obj, name):
    """ Describe the object passed as argument"""

    if isinstance(obj, dict):
        obj = ('\n' + ' '*(INDENT+1)).join(["{}: {}".format(k, obj[k]) for k in obj])
        print
    elif isinstance(obj, list):
        obj = ('\n' + ' '*(INDENT+1)).join(["{}".format(k) for k in obj])
        print
    wi('<%s> %s = %s' % (obj.__class__.__name__, name, obj))
    print
...
def describe(module):
    ...
    for name in dir(module):
       ...
       if inspect.ismodule(obj):
          ...
          count += 1; describe_other(obj, name)
